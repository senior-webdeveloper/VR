...
function render_table_rows(rows, req_per_page, page_no) {
  ...
  const resp = response.slice(
    req_per_page * (page_no - 1),
    req_per_page * page_no
  );
  $("#request-table").empty();
  $("#request-table").append(
    "<tr><th>Index</th><th>Request No</th><th>Title</th></tr>"
  );
...
function pagination(data, myarr) {
  ...
  if (data.req_per_page !== "ALL") {
    let pager =
      `<a href="#" id="prev_link" onclick=active_page('prev',\"${all_data}\",${data.req_per_page})>&laquo;</a>` +
      `<a href="#" class="active" onclick=active_page(this,\"${all_data}\",${data.req_per_page})>1</a>`;
    const total_page = Math.ceil(
      parseInt(myarr.length) / parseInt(data.req_per_page)
    );
